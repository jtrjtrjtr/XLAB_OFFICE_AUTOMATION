...
## Step 1 — Ask (max 5 questions, in one go)
...
4. **Vstupy** — co půjde do `inputs/` (např. PPTX briefy, maily, data)?
5. **Opakovaně?** — má z toho být i scheduled task (ano/ne, jak často)?
...
## Kdo jsem
- Jindřich Trapl — CEO XLAB (kreativní tech / immersive eventy). Jazyk: česky, stručně.
...
- Cituj zdroje (soubor/URL). Nejistota → zeptej se, neimprovizuj.

## ⚠️ Bezpečnost souborů (SharePoint sync)
- Nikdy nemaž/nepřepisuj zdroje v inputs/ a references/ — jsou read-only.
- Zapisuj jen do outputs/, done/, change-log.md. Měníš výstup → verzuj (_v2), nepřepisuj naslepo.

## Anti-patterns
- Nezakládej soubory mimo tuhle složku. Nic ven (mail/3. strana) bez pokynu. Nevymýšlej fakta — chybí → "TBD".
```
...
Krátký text (elevator pitch projektu) k vložení do pole "Project Instructions" v Cowork UI. Odkaž na CLAUDE.md a styl (česky, stručně, jen uvnitř složky, vstupy read-only).
